...
export default function DCASimulator() {
  ...
  // Auto risk offset from realized volatility (percentile-based per asset)
  // Computes 60-day vol, ranks against asset's own full historical vol range
  // Low percentile (calm for THIS asset) = aggressive offset, high = cautious
  const autoRiskOffset = useMemo(() => {
    if (dailyData.length < 120) return -0.05;

    // Compute rolling 60-day vol for entire history
    const VOL_WINDOW = 60;
    const allVols = [];
    for (let start = 0; start <= dailyData.length - VOL_WINDOW - 1; start++) {
      const slice = dailyData.slice(start, start + VOL_WINDOW + 1);
      const returns = [];
      for (let j = 1; j < slice.length; j++) {
        if (slice[j].price > 0 && slice[j-1].price > 0) {
          returns.push(Math.log(slice[j].price / slice[j-1].price));
        }
      }
      if (returns.length < 30) continue;
      const mean = returns.reduce((a, b) => a + b, 0) / returns.length;
      const variance = returns.reduce((a, r) => a + (r - mean) ** 2, 0) / returns.length;
      allVols.push(Math.sqrt(variance) * Math.sqrt(365));
    }
    if (allVols.length < 10) return -0.05;

    // Current 60-day vol (last window)
    const currentVol = allVols[allVols.length - 1];

    // Rank current vol as percentile of this asset's own history
    const sorted = [...allVols].sort((a, b) => a - b);
    let rank = 0;
    for (let i = 0; i < sorted.length; i++) {
      if (sorted[i] <= currentVol) rank = i;
    }
    const percentile = rank / (sorted.length - 1); // 0 = lowest vol, 1 = highest

    // Map percentile to offset:
    //   0th pct (calmest for this asset) -> -0.10 (aggressive, buy more)
    //   50th pct (normal)                -> -0.055 (moderate)
    //   100th pct (most volatile)        -> -0.01 (cautious)
    const offset = -0.10 + percentile * 0.09;
    return Math.min(-0.01, Math.max(-0.12, parseFloat(offset.toFixed(3))));
  }, [dailyData]);
  ...
  return (
    <div style={{ background: T.bg, minHeight: "100vh", color: T.text, fontFamily: "'DM Mono', monospace", padding: "24px 28px 80px 28px" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .np-disclaimer { position: fixed; bottom: 0; left: 0; right: 0; z-index: 9999; background: #0d0d1f; border-top: 1px solid #2a2a4a; padding: 5px 12px; cursor: pointer; }
      ...
      {(() => {
        ...
        return (
          <div className="np-disclaimer" onClick={() => setExpanded(e => !e)}>
            <div className="np-disclaimer-text">
              ⚠️ Not financial advice · Educational only · © 2026 North Prosperity
              <span style={{ color: "#6C8EFF", marginLeft: 6 }}>{expanded ? "▲ less" : "▼ more"}</span>
            </div>
            {expanded && (
              <div className="np-disclaimer-text" style={{ marginTop: 3 }}>
                For educational and simulation purposes only · Past performance does not guarantee future results · Always consult a qualified financial advisor before making investment decisions
              </div>
            )}
          </div>
        );
